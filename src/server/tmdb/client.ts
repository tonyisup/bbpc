...
const API_BASE_URL = "https://api.themoviedb.org/3",
  SEARCH_BASE_URL = `${API_BASE_URL}/search/movie?api_key=${process.env.TMDB_API_KEY}&language=en-US`,
  SEARCH_TV_URL = `${API_BASE_URL}/search/tv?api_key=${process.env.TMDB_API_KEY}&language=en-US`,
...
export const tmdb = {
  ...
  getMovie: async (id: number): Promise<Title> => {
    const resp = await fetch(`${API_BASE_URL}/movie/${id}?api_key=${process.env.TMDB_API_KEY}&language=en-US`)
    const res = await resp.json()
    const title = {
      ...res,
  ...
  getShow: async (id: number): Promise<Title> => {
    const resp = await fetch(`${API_BASE_URL}/tv/${id}?api_key=${process.env.TMDB_API_KEY}&language=en-US&append_to_response=external_ids`)
    const res = await resp.json()
    const title = {
      ...res,
      title: res.name,
