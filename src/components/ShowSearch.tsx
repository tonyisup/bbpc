...
interface ShowSearchProps {
	setShow: Dispatch<SetStateAction<Title | null>>;
}

const ShowSearch: FC<ShowSearchProps> = ({ setShow }) => {
	const [modalOpen, setModalOpen] = useState<boolean>(false);
	const [searchQuery, setSearchQuery] = useState<string>("");
	const { data: resp } = trpc.show.search.useQuery({
		page: 1,
				...
				<div className="text-white w-full inset-0 flex items-center justify-center bg-black/75">
					<div className="p-3 w-full bg-gray-800">
						<div className="relative w-full flex">
							<span className="text-2xl font-semibold">Show Search</span>
							<button
								type="button"
								title="Close"
								onClick={closeModal}
						...
						<div>
							<Input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search for a show" />
							<CarouselProvider
								...
								<Slider>
									{resp && resp.results.map((title, index) => (
										<Slide index={index} key={title.id}>
											<button type="button" title="Select this show" onClick={() => selectTitle(title)}>
