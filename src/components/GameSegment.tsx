      ...
      <>
        <div className="m-4 flex flex-col items-center gap-4">
          <p className="text-2xl">
            Please <SignInButton /> to submit guesses
          </p>
        </div>
      </>
    );
    ...
    <div className="flex flex-col items-center gap-4 py-4">
      ...
      <div className="flex flex-col items-center gap-4 md:flex-row">
        ...
        >
          ...
          <HiPhone className="m-2 inline-block" />
        </Button>
        <Button
          variant="outline"
        ...
        >
          ...
          <HiMicrophone className="m-2 inline-block" />
        </Button>
        <Button
          variant="outline"
        ...
        >
          Click some buttons
          <GamepadIcon className="m-2 inline-block" />
        </Button>
      </div>
      {gameChoice == GameChoice.PhoneMessage && <PhoneNumber />}
      {gameChoice == GameChoice.VoiceRecording && (
        <RecordAssignmentAudio
          userId={userData?.id}
          assignmentId={assignment.id}
        />
      )}
      {gameChoice == GameChoice.ClickButtons && (
        <GamePanel session={session} assignment={assignment} />
      )}
    </div>
  );
};
...
const GamePanel: FC<GamePanelProps> = ({ session, assignment }) => {
  const { data: guesses, refetch } =
    api.review.getGuessesForAssignmentForUser.useQuery({
      assignmentId: assignment.id,
    });
  ...
  return (
    <>
      {session && guesses?.length != undefined && guesses.length > 0 && (
        <ShowAssignmentGuesses
          guesses={guesses}
          resetGuesses={handleResubmitGuesses}
        />
      )}
      {session && (guesses?.length == 0 || setGuesses) && (
        <SetAssignmentGuesses
          assignment={assignment}
          guessesSaved={handleSavedGuesses}
        />
      )}
    </>
  );
...
const ShowAssignmentGuesses: FC<ShowAssignmentGuessesProps> = ({
  guesses,
  resetGuesses,
}) => {
  return (
      ...
      {guesses &&
        guesses.map((guess) => {
          return (
            <SelectedGuess
              key={guess.id}
              host={guess?.assignmentReview?.review?.user}
              rating={guess?.rating}
            />
          );
        })}
      <Button variant="outline" onClick={resetGuesses}>
        <HiRefresh className="m-2 inline-block" />
        Redo Picks!
      </Button>
    </div>
  );
...
const SetAssignmentGuesses: FC<SetAssignmentGuessesProps> = ({
  ...
  return (
    <div className="flex flex-col items-center gap-4 py-4">
      ...
      >
        <HiUpload className="m-2 inline-block" />
        Submit Picks!
      </Button>
      {pendingGuesses &&
        Object.keys(pendingGuesses).map((key) => {
          const guess = pendingGuesses[key];
          return (
            <SelectedGuess
              key={key}
